 ...
 */
async function storeBookmarks(searchItems: SearchItem[]) {
  try {
 ...
 */
async function loadBookmarks(): Promise<SearchItem[] | null> {
  try {
...
export async function doSearch(query: string): Promise<SearchResult[]> {
  ...
  // 从本地获取书签，如果没有则重新获取
  const storedData = await loadBookmarks()
  ...
  if (storedData) {
    ...
    // 将搜索项保存到本地存储
    await storeBookmarks(searchItems)
  }
  ...
  switch (mode) {
    case "all":
      ...
      const allTabsSearchResult: SearchResult[] = allTabs.map((tab) => {
        ...
        if (tab.favIconUrl) {
          const urlProtocol = new URL(tab.favIconUrl).protocol
