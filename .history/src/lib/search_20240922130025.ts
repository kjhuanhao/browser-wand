...
export const doSearch = async (
  ...
  const searchIntel: SearchItem = {
    id: "searchIntel",
    title: "互联网搜索",
    url: `https://www.baidu.com/search?q=`,
    iconUrl: "https://s2.loli.net/2024/09/22/oflIVsGZW62HU4m.png",
    type: "search"
  }
  searchItems.push(searchIntel)
  // 从本地获取书签，如果没有则重新获取
  const storedData = await loadFlexSearchData()
  if (storedData) {
    searchItems = [...searchItems, ...storedData]
  } else {
    // TODO 策略模式, 第一项应该都是Search 行为
    const bookmarks = await fetchBookmarks()
    searchItems = [...searchItems, ...bookmarks]
    // 将搜索项保存到本地存储
