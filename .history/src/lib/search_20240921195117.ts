...
export const doSearch = async (
  searchItems: SearchItem[],
  value: string
): Promise<(SimpleDocumentSearchResultSetUnit & Partial<SearchItem>)[]> => {
  // 创建索引
  let index = createIndex(searchItems)
  console.log(index, "123")

  // 执行搜索
  const searchResults = index.search(value, { limit: 10, suggest: true })

  // 用于存储所有完整的结果
  const fullResults: (SimpleDocumentSearchResultSetUnit &
    Partial<SearchItem>)[] = []
