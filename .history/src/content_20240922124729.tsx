...
const PlasmoOverlay = () => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.ctrlKey && event.key === "k") {
        ...
        setSelectedIndex((prevIndex) =>
          prevIndex === -1 ? (searchResult?.length || 0) - 1 : prevIndex - 1
        )
      } else if (event.key === "ArrowDown") {
        setSelectedIndex((prevIndex) =>
          prevIndex === (searchResult?.length || 0) - 1 ? -1 : prevIndex + 1
        )
      }
    }
  ...
  const debouncedSearch = _.debounce(async (searchValue: string) => {
    ...
    if (result.length == 0) {
      ...
      setSelectedIndex(-1) // 当没有结果时，选中互联网搜索
      return
    }
    setSearchResult(result[0].result as any)
    setSelectedIndex(-1) // 即使有结果，也默认选中互联网搜索
    console.log(result)
  }, 100)
      ...
      <div className="plasmo-m-auto plasmo-bg-white plasmo-p-6 plasmo-rounded-lg plasmo-shadow-xl plasmo-max-h-[540px] plasmo-w-[700px]">
        ...
        <div className="plasmo-flex plasmo-flex-col plasmo-mt-4 plasmo-border-b">
          <div>
            ...
            <div
              className={cn(
                SearchItemVariants({ active: selectedIndex === -1 })
              )}
              onClick={() => setSelectedIndex(-1)}>
              <p className="plasmo-text-lg">🔍 互联网搜索</p>
            </div>
            {/* 搜索结果 */}
