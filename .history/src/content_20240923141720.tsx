...
const PlasmoOverlay = () => {
  ...
  useEffect(() => {
    const anchors = document.querySelectorAll("#plasmo-shadow-container")
    console.log(anchors, " test")

    if (searchItemRefsRef.current[selectedIndex]?.current) {
      searchItemRefsRef.current[selectedIndex].current.scrollIntoView({
        behavior: "smooth",
        block: "nearest"
      })
    }
  }, [selectedIndex, searchItemRefsRef.current])
