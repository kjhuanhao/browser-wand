...
const PlasmoOverlay = () => {
  ...
  const memoizedAssembleSearchResult = React.useMemo(() => {
    return async (searchResult: SearchResult[], mode: SearchMode) => {
      return await assembleSearchResult(searchResult, mode)
    }
  }, [])
  useEffect(() => {
    memoizedAssembleSearchResult([], "all")
  }, [])
  useEffect(() => {
  ...
  const handleSearch = async (e: any) => {
    ...
    setSearchResult(initializeResult)
    setSelectedIndex(0)
    console.log(initializeResult, "init")

    // 在这里执行 fetchData
    if (initializeResult.length === 1) {
      console.log(initializeResult, "tttt")
      const allResult = await memoizedAssembleSearchResult([], "all")
